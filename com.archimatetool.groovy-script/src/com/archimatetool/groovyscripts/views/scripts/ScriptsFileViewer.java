...
import com.archimatetool.groovyscripts.views.file.AbstractFileView;
import com.archimatetool.groovyscripts.views.file.FileTreeViewer;
import com.archimatetool.groovyscripts.views.file.NewFileDialog;
import com.archimatetool.groovyscripts.views.file.PathEditorInput;
...
extends AbstractFileView  {
    ...
    protected void makeActions() {
        ...
        // Icon
        fActionNewFile.setImageDescriptor(IArchiScriptImages.ImageFactory.getImageDescriptor(IArchiScriptImages.ICON_GROOVY));
        fActionNewFileDeps.setImageDescriptor(IArchiScriptImages.ImageFactory.getImageDescriptor(IArchiScriptImages.ICON_DEPS));
    }
        ...
        if(parent.exists()) {
            ...
            if(dialog.open()) {
                ...
                if(newFile != null) {
                    ...
                    try {
                   		URL urlNewFile = ArchiScriptPlugin.INSTANCE.getBundle().getEntry("templates/new.groovy"); //$NON-NLS-1$                    		
	...
	@Override
	protected void handleNewFileDepsAction() {
		
        File parent = getRootFolder();
        File newFile = new File(ArchiScriptPlugin.INSTANCE.getPreferenceStore().getString(IPreferenceConstants.PREFS_SCRIPTS_FOLDER) + File.separator + ".deps");
        try {
           	boolean replace = true;
        	if (newFile.exists()) {
        		replace = MessageDialog.openConfirm(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Archi", Messages.Confirm_Deps_Replacement_0);
        	}
        	if (replace) {
	       		URL urlNewFile = ArchiScriptPlugin.INSTANCE.getBundle().getEntry("templates/.deps"); //$NON-NLS-1$                    		
	            InputStream in = urlNewFile.openStream();
	            Files.copy(in, newFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
	            in.close();
        	}
